...
import {
  ...
  ref,
  getDownloadURL,
} from "https://www.gstatic.com/firebasejs/10.13.1/firebase-storage.js"; // ใช้สำหรับ Storage
  ...
  reports.forEach((report, index) => {
    if (report.re_image && report.re_image.length > 0) {
      ...
      butdelete.addEventListener("click", async (e) => {
        ...
        console.log(report.re_userwasreported);
...
async function deleteUserAndPosts(user_name) {
  try {
    const userDoc = doc(db, "users", user_name); 
    console.log(userDoc);
    ...
    userSnapshot.docs.forEach(async (user) => {
      if (user.data().user_name === user_name) {
    ...
    postSnapshot.docs.forEach(async (post) => {
      if (post.data().p_username === user_name) {
